...
const DashboardArticles: React.FC = () => {
  ...
  const [articles, setArticles] = useState<Article[]>([{
    ...
    descripcion: "Un teléfono inteligente de última generación con múltiples funciones.",
    fechaRegistro: "01/09/2023",
  }]);
  const [services, setServices] = useState<Service[]>([{
    ...
    descripcion: "Asesoramiento experto en tecnología de la información para optimizar procesos empresariales.",
    requisitos: "Reunión inicial para evaluar necesidades.",
  ...
  const [modalMode, setModalMode] = useState<"article" | "service" | null>(
    null
  );
  ...
  const initialUnion = Array.from(
    new Set([...articleCategories, ...serviceTypes])
  );
  ...
  useEffect(() => {
    ...
    //             items = (data as string[]).map((n) => ({
    //               name: n,
    //               allowed: true,
    //             }));
    //           else
    //             items = (data as { name?: string; allowed?: boolean }[]).map(
    //               (d) => ({
    ...
    //         ) {
    //           items = (
    //             data as { categories: { name?: string; allowed?: boolean }[] }
    //           ).categories.map((d) => ({
    //             name: d.name ?? String(d),
    //             allowed: d.allowed ?? true,
    //           }));
  ...
  const handleArticleSubmit = async () => {
    ...
    //     showMessage(
    //       "No tienes permiso para usar la categoría seleccionada",
    //       "error"
    //     );
    //     return;
    //   }
  ...
  const handleServiceSubmit = async () => {
    ...
    //   if (found && found.allowed === false) {
    //     showMessage(
    //       "No tienes permiso para usar el tipo de servicio seleccionado",
    //       "error"
    //     );
    //     return;
    //   }
  ...
  const filteredItems = (
    [...articles, ...services] as (Article | Service)[]
  ).filter((item) => {
        ...
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-gray-900">
              Artículos y Servicios Registrados
            </h3>
              ...
              >
                ...
                {categories.map((cat) => (
                  <option
                    key={cat.name}
                    value={cat.name}
                    disabled={cat.allowed === false}
                  >
                    {cat.name}
                    {cat.allowed === false ? " (sin permiso)" : ""}
                  </option>
                ))}
              </select>
              <button
                ...
                className="ml-3 inline-flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700"
              >
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500">
                {searchTerm || filterCategory
                  ...
                  : "No hay artículos o servicios registrados"}
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              {filteredItems.map((item) => (
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <div className="text-right">
                      <p className="font-semibold text-green-600 text-lg">
                        ${(item as Article).precio.toLocaleString()}
                        {(item as Service).tipo ? "/hora" : ""}
                      </p>
                    </div>
                  </div>
                    ...
                    {(item as Article).categoria !== undefined ? (
                      <>
                        <div>
                          ...
                          {(item as Article).categoria || "Sin categoría"}
                        </div>
                        <div>
                          <span className="font-medium">Stock:</span>{" "}
                          {(item as Article).stock}
                        </div>
                        <div>
                          <span className="font-medium">Proveedor:</span>{" "}
                          {(item as Article).proveedor || "No especificado"}
                        </div>
                        <div>
                          <span className="font-medium">Registrado:</span>{" "}
                          {(item as Article).fechaRegistro}
                        </div>
                      </>
                    ) : (
                      <>
                        <div>
                          ...
                          {(item as Service).tipo || "Sin tipo"}
                        </div>
                        <div>
                          <span className="font-medium">Duración:</span>{" "}
                          {(item as Service).duracion} hrs
                        </div>
                        <div>
                          <span className="font-medium">Responsable:</span>{" "}
                          {(item as Service).responsable || "No asignado"}
                        </div>
                        <div>
                          <span className="font-medium">Registrado:</span>{" "}
                          {(item as Service).fechaRegistro}
                        </div>
                      </>
                    )}
                  </div>
                  ...
                  {(item as Service).tipo !== undefined &&
                    (item as Service).requisitos && (
              ...
              <div>
                ...
                <div className="space-y-3">
                  <SimpleInput
                    ...
                    onChange={(e) =>
                      handleArticleFormChange("codigo", e.target.value)
                    }
                  ...
                  <SimpleInput
                    ...
                    onChange={(e) =>
                      handleArticleFormChange("nombre", e.target.value)
                    }
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleArticleFormChange("categoria", e.target.value)
                      }
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <SimpleInput
                      ...
                      onChange={(e) =>
                        handleArticleFormChange("precio", e.target.value)
                      }
                      placeholder="0.00"
                    />
                    <SimpleInput
                      ...
                      onChange={(e) =>
                        handleArticleFormChange("stock", e.target.value)
                      }
                      placeholder="0"
                    />
                  </div>
                  <SimpleInput
                    ...
                    onChange={(e) =>
                      handleArticleFormChange("proveedor", e.target.value)
                    }
              ...
              <div>
                ...
                <div className="space-y-3">
                  <SimpleInput
                    ...
                    onChange={(e) =>
                      handleServiceFormChange("codigo", e.target.value)
                    }
                  ...
                  <SimpleInput
                    ...
                    onChange={(e) =>
                      handleServiceFormChange("nombre", e.target.value)
                    }
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleServiceFormChange("tipo", e.target.value)
                      }
                      className="text-slate-900 bg-white border border-slate-300 w-full text-sm p-2 mt-1 rounded-md focus:outline-blue-500"
                    >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <SimpleInput
                      ...
                      onChange={(e) =>
                        handleServiceFormChange("precio", e.target.value)
                      }
                      placeholder="0.00"
                    />
                    <SimpleInput
                      ...
                      onChange={(e) =>
                        handleServiceFormChange("duracion", e.target.value)
                      }
                      placeholder="0"
                    />
                  </div>
                  <SimpleInput
                    ...
                    onChange={(e) =>
                      handleServiceFormChange("responsable", e.target.value)
                    }
